...
#    print len(sub1)
    ...
    for sub in sub1:
      plt.clf()
      x,y,vr,gm  = pretty_picture(out_dir,snap,sub,sub_cat,box_size,scf,names[idx],mass[sub,4],fullGroup=True)
...
def pretty_picture(out_dir, snap, sub, sub_cat, box_size, scf, name, mass, fullGroup = False):
      ...
      if fullGroup == False:
        ...
        print 'cat'
        gas_pos  = il.snapshot.loadHalo(out_dir, snap, GrNr, 0, fields = ['Coordinates' ])
        gas_vel  = il.snapshot.loadHalo(out_dir, snap, GrNr, 0, fields = ['Velocities' ])
        gas_mass = il.snapshot.loadHalo(out_dir, snap, GrNr, 0, fields = ['Masses' ])
        gas_sfr  = il.snapshot.loadHalo(out_dir, snap, GrNr, 0, fields = ['StarFormationRate'])
        gas_rho  = il.snapshot.loadHalo(out_dir, snap, GrNr, 0, fields = ['Density' ])
        gas_met  = il.snapshot.loadHalo(out_dir, snap, GrNr, 0, fields = ['GFM_Metallicity' ])
        gas_eab  = il.snapshot.loadHalo(out_dir, snap, GrNr, 0, fields = ['ElectronAbundance'])
        gas_ien  = il.snapshot.loadHalo(out_dir, snap, GrNr, 0, fields = ['InternalEnergy' ])
        ZO       = il.snapshot.loadHalo(out_dir, snap, GrNr, 0, fields = ['GFM_Metals' ])[:,4]
        XH       = il.snapshot.loadHalo(out_dir, snap, GrNr, 0, fields = ['GFM_Metals'       ])[:,0]
...
#      plt.scatter(gas_pos[:,0],gas_pos[:,1],s=1.00E-03)

      plt.hist2d(gas_pos[:,0],gas_pos[:,1],bins=(500,500),norm=mpl.colors.LogNorm())

      plt.savefig('./Pretty/full_group')
